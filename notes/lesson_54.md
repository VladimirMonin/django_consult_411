...
коммит: lesson_54: реристрация модели в админке
...
Пояснить про:
* Почему для бизнеса важно НЕ удалять данные при удалении мастера
* Что будет происходить с записью при удалении мастера
...
Так как мы специально не стали для внешнего ключа добавлять `blank=True` то при создании записи в админке мы не сможем оставить поле пустым.

![order_detail_form.png](./images/order_detail_form.png)

Коммит: lesson_54: модель Order
...
Установка shell_plus `poetry add django-extensions`

Его нужно подключить в настройках проекта:
