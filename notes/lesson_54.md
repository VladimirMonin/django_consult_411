...
# Эта команда возвращает QuerySet - это коллекция объектов, которая еще не выполнила запрос к базе данных.
# Запрос к базе данных выполняется только тогда, когда вы пытаетесь получить доступ к элементам QuerySet.

# Если попросить orders[0] - Мы получаем из коллекции экземпляр Order
# Например, orders[0] вернет первый объект Order из QuerySet.

# orders[0].name - Получаем данные из поля конкретного экземпляра
# Например, orders[0].name может вернуть "Фродо", если это имя первого заказа.

# orders[0].master.first_name - Получаем данные из связанной модели
# Это демонстрирует, как легко получить доступ к полям связанной модели (Master) через внешний ключ.
# Например, orders[0].master.first_name вернет имя мастера, связанного с первым заказом.
```
